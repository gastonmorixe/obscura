...
async fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        Some(Command::Fetch { url, dump, selector, wait, timeout, wait_until, user_agent, stealth, eval, output, quiet }) => {
            ...
            run_fetch(&url, dump, selector, wait, timeout, &wait_until, user_agent, stealth, eval, output, quiet, global_proxy, extension_runtime.clone()).await?;
        }
...
async fn run_fetch(
    ...
    let context = {
        let mut ctx = BrowserContext::with_options("fetch".to_string(), proxy, stealth);
